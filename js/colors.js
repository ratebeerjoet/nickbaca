...
function minimumContrastRatio() {
  ...
  const scaledFontSize = getFontSizeByDevice.call(this, fontSize, forceBreakpoint) * DEFAULT_WEBSITE_FONT_SCALES[fontScale];
...
function accessible(color) {
  ...
  if (Color.contrastRatio(color, background) < contrastRatioThreshold) {
    const nearestAccessible = first(getNearestAccessibleColors(contrastRatioThreshold, background, color));
...
function colorPrimary() {
  const [primary, background, multiplier] = utils.getDial(this, ['primary', 'background', 'multiplier']);
  return primary.contrastWith(background, 40 * multiplier, true);
}

function colorAccent() {
  const [accent, background, multiplier] = utils.getDial(this, ['accent', 'background', 'multiplier']);
  return accent.contrastWith(background, 40 * multiplier, true);
}

function colorNeutral() {
  const [neutral, background, multiplier] = utils.getDial(this, ['neutral', 'background', 'multiplier']);
  return neutral.contrastWith(background, 40 * multiplier, true);
}

function colorError() {
  const [background, multiplier, error] = utils.getDial(this, ['background', 'multiplier', 'error']);
...
function colorHighlight() {
  const [background, multiplier, highlight, primary] = utils.getDial(this, ['background', 'multiplier', 'highlight', 'primary']);
...
function colorHighlightActive() {
  const [background, multiplier, highlight] = utils.getDial(this, ['background', 'multiplier', 'highlight']);
...
function colorInputHighlight() {
  const backgroundColorInput = utils.getBackgroundColor(this, 'input');
  const multiplier = utils.getDial(this, 'multiplier');
  return utils.getColor(this, 'highlight').contrastWith(backgroundColorInput, 30 * multiplier, true);
}
