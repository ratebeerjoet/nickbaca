...
class ThumbnailNavList extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    const leftOffSet = conveyorMarginLeft + selectedIndex * (thumbWidth + thumbMargin);
    if (leftOffSet < 0) {
      ...
      this.moveConveyor('right', Math.ceil((leftOffSet + thumbWidth - conveyorWidth) / stepSize))();
    }
  }
  ...
  get thumbsExceededWidth() {
    ...
    return thumbWidth * images.length + thumbMargin * (images.length - 1) - conveyorWidth;
  }
  ...
  renderThumbnails() {
    const { images, selectedIndex, goToSlide, thumbWidth, thumbMargin } = this.props;
    const { conveyorMarginLeft, conveyorWidth } = this.state;
    let thumbArray = null;
    ...
    if (conveyorWidth) {
      ...
      const lastVisibleIndex = firstVisibleIndex + Math.ceil(conveyorWidth / fullThumbWidth) + 1;
    ...
    return (
      <UX2.Element.Block style={{ position: 'relative' }}>
        ...
        <ul style={ listStyles } data-aid={ dataAids.THUMBNAIL_NAV_LIST } children={ thumbArray } />
  ...
  getArrowStyle(direction) {
    ...
    if (
      ...
      (direction === 'right' && width !== Math.abs(conveyorMarginLeft) && width > 0)
    ) {
      merge(arrowStyle, {
        position: 'absolute',
        zIndex: 1,
        ...
        height: thumbHeight,
        width: 50,
        background: transparentBackground ? 'transparent' : 'rgba(0, 0, 0, 0.5)',
        padding: 0,
        cursor: 'pointer',
        outline: 'none',
        top: 0,
  ...
  moveConveyor = (direction, stepNum = 1) => {
    return event => {
      ...
      if (direction === 'left' && absMargin < step) {
        step = absMargin;
      } else if (direction === 'right' && this.thumbsExceededWidth - absMargin < step) {
