  ...
  class Gallery extends Component {
    constructor(props) {
      ...
      this.nextSlideDebounced = debounce(
        input => handleDirectionalKeys({ keyCode: input.keyCode, context: this }),
        200
      );
    }
    ...
    renderFakeCarousel(images) {
      ...
      return (
        <UX2.Element.Block color='highlight'>
          <UX2.Element.Block
            style={{ width: 900, overflow: 'hidden', margin: '0 auto', whiteSpace: 'nowrap' }}
          >
            {cloneElement(image1, {
              style: merge({}, commonStyles, { margin: '0 5px 0 -605px', opacity: 0.3 })
            })}
            {cloneElement(image2, { style: commonStyles })}
            {cloneElement(image3, { style: merge({}, commonStyles, { opacity: 0.3 }) })}
          </UX2.Element.Block>
          <UX2.Element.Block style={{ width: 900, overflow: 'hidden', margin: '0 auto' }}>
            {createElement(Caption, {
              maxWidthCaption,
              captionContainerHeight: this.props.hasCaption ? CAPTION_HEIGHT : 0,
    ...
    componentDidUpdate(prevProps) {
      ...
      if (currentImageSelected !== prevProps.currentImageSelected && currentImageSelected > -1) {
    ...
    renderCarousel(imageComponents = [], dimensions, showArrows) {
      ...
      const resolvedIndex =
        currentImageSelected > -1 ? parseInt(currentImageSelected, 10) : selectedIndex;
      const initialSlideProp = { initialSlide: parseInt(resolvedIndex, 10) };
      const caption = (images[selectedIndex] && images[selectedIndex].caption) || '';
      ...
      if (hasCaption) {
        controls.push({
          component: Caption,
          props: { maxWidthCaption, captionContainerHeight: CAPTION_HEIGHT, caption },
          position: 'bottom'
        });
      }
      const carouselHeight =
        fullBleed && !showthumbnails && !hasCaption ? `${dimensions.height}px` : 'auto';
      ...
      const imagesToPrefetchOverride = fullBleed ? { imagesToPrefetch: 10 } : {};
    ...
    render() {
      const { device, images, renderAsThumbnail, size, fullBleed, showArrows } = this.props;

      const Grid = UX2.Component.Grid;
      const isSmallTablet = device === TABLET && size === 'sm';
      const dimensions = isSmallTablet ? DIMENSIONS.smallTablet : DIMENSIONS[device];
      ...
      const imageArray = images.map(({ imageData }, index) => {
        ...
        const imageUrl = utils.generateImageServiceUrl({
          ...imageData,
          additionalUrlParams: [`rs=w:${maxDimensions.width},h:${maxDimensions.height}`]
        });
        const imgStyle = {
          maxWidth: '100%',
          position: 'relative',
        ...
        return (
          <UX2.Element.Image
            ...
            key={ index }
            style={ imgStyle }
            onMouseEnter={ modifyKeyDownEventHandler.bind(this, this.nextSlideDebounced, true) }
            onMouseLeave={ modifyKeyDownEventHandler.bind(this, this.nextSlideDebounced) }
      ...
      if (imageArray.length) {
        const renderArrows = showArrows && device !== 'mobile' && imageArray.length > 1;
        ...
        carouselContainer = fullBleed ? carouselInner : <Grid size={ 1 }>{carouselInner}</Grid>;
      }

      const CarouselWrapper = fullBleed ? UX2.Element.Block : UX2.Element.Container;
      return <CarouselWrapper data-test-radpack='bootstrap2'>{carouselContainer}</CarouselWrapper>;
    }
  }
);
